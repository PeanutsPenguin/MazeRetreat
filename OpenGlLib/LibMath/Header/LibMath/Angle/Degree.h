...
namespace LibMath
{
	...
	{
	public:
		Degree(void) : m_value(0.f) {}
		explicit Degree(float raw) : m_value(raw) {}						// explicit so no ambiguous / implicit conversion from float to angle can happen
		Degree(Degree const&) = default;
		~Degree(void) = default;

		operator Radian(void) const;					// Radian angle = Degree{45};		// implicit conversion from Degree to Radian
