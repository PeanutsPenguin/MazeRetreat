...
namespace LibMath
{
	...
	{
		return LibMath::acos(LibMath::Vec4::Dot(LibMath::Vec4::Normalize(a), LibMath::Vec4::Normalize(b)));
	}
	...
	{
		return b * (LibMath::Vec4::Dot(a, b) / LibMath::Vec4::Dot(b, b));
	}

	template<typename _Type>
	inline Vector<4, _Type> LibMath::Vector<4, _Type>::Reflect(const Vector<4, _Type>& a, const Vector<4, _Type>& b)
	{
		LibMath::Vec4 N = LibMath::Vec4::Normalize(b);
		return a - N * LibMath::Vec4::Dot(N, a) * static_cast<_Type>(2);
	}
